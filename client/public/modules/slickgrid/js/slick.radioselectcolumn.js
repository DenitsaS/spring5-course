(function ($) {
  ...
  function RadioSelectColumn(options) {
    ...
    function handleClick(e, args) {
      if (_grid.onBeforeRowChange && args.row != _grid.getSelectedRows()[0]) {
        _grid.onBeforeRowChange(e, args);
      }
    ...
    function checkboxSelectionFormatter(row, cell, value, columnDef, dataContext) {
      if (dataContext) {
        return _selectedRowsLookup[row]
          ? "<input type='radio' name='" + columnDef.id + "-name' checked='checked'>"
          : "<input type='radio' name='" + columnDef.id + "-name'>";
      }
      return null;
    }
