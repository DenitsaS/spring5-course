(function ($) {
    ...
    */
    ...
    function HeaderFilter(options) {
        ...
        var defaults = {
            buttonImage: "img/slickgrid/down.png",
            filterImage: "img/slickgrid/filter.png",
            sortAscImage: "img/slickgrid/sort-asc.png",
            sortDescImage: "img/slickgrid/sort-desc.png"
        };
        var $menu;
        ...
        function addMenuInput(menu, columnDef) {
            $("<input class='input form-control' placeholder='Search' style='margin-top: 5px; width: 206px'>")
                .data("column", columnDef)
        ...
        function updateFilterInputs(menu, columnDef, filterItems) {
            var filterOptions = "<label style='display: -webkit-box; cursor: pointer;'>"
                + "<input style='cursor: pointer;' type='checkbox' value='-1' /><div style='margin-top: 3px; margin-left: 3px;'>( Select All )</div></label>";
            ...
            for (var i = 0; i < filterItems.length; i++) {
                ...
                filterOptions += "<label style='display: -webkit-box; cursor: pointer;'><input style='cursor: pointer;' type='checkbox' value='" + i + "'"
                    + (filtered ? " checked='checked'" : "")
                    + "/><div style='margin-top: 4px; margin-left: 3px;'>" + filterItems[i] + "</div></label>";
            }
        ...
        function showFilter(e) {
            ...
            for (var i = 0; i < filterItems.length; i++) {
                let item = filterItems[i] instanceof Object ? filterItems[i].name : filterItems[i];
                var filtered = _.contains(workingFilters, item);
                filterOptions += "<label style='display: -webkit-box; cursor: pointer;'><input style='cursor: pointer;' type='checkbox' value='" + i + "'"
                    + (filtered ? " checked='checked'" : "")
                    + "/><div style='margin-top: 4px; margin-left: 3px;'>" + item + "</div></label>";
            }

            var $filter = $("<div class='filter' style='width:206px'>")
                .append($(filterOptions))
                .appendTo($menu);

            $('<button id="ok_btn" class="btn btn-primary" style="background-color:#2AA689; width:66px">OK</button>')
                .appendTo($menu)
            ...
            $('<button class="btn btn-warning" style="background-color:#f8ac59; width:66px">Clear</button>')
                .appendTo($menu)
                .bind('click', function (ev) {
            ...
            $('<button class="btn btn-primary" style="background-color:#2AA689; width:66px">Cancel</button>')
                .appendTo($menu)
                .bind('click', hideMenu);
